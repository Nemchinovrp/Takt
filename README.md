...
Веб-терминал на Python: **Tinkoff Invest SDK + vn.py + PySide6**.
Портфель, котировки, график цены, активные заявки и журнал в одном окне.

![Обзор портфеля](docs/screenshots/overview.png)
...
Откройте http://127.0.0.1:3729 в браузере. Веб-интерфейс запускается по умолчанию; прежнее окно Qt доступно с `--desktop`.

Запуск с добавленным токеном для чтения настоящего счёта:

```bash
uv run pyveighna --mode readonly --port 3729
```
...
| `INVEST_FIGIS` | 1–20 FIGI через запятую; по умолчанию SBER, GAZP, LKOH, YDEX |
...
```text
Tinkoff Client / SandboxClient / DemoProvider
...
12 секунд; при закрытии окна новые вызовы прекращаются, текущий может завершаться до deadline.
...
Тесты не отправляют запросы с настоящим токеном. Скриншот всегда создаётся в деморежиме.
...
Актуальная доступность методов архивного SDK на настоящем счёте должна быть проверена
с вашим токеном: в этой сборке интеграция проверена локально и на подменённых ответах API.
