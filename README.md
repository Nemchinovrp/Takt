...
## Демонстрационная стратегия SMA

Под графиком доступна панель «Стратегия · Пересечение SMA». Выберите инструмент
в списке котировок, задайте быструю и медленную средние и нажмите «Применить».
По умолчанию используются **10 и 30 завершённых пятиминутных свечей**.
Допустимые периоды: `2 ≤ быстрая < медленная ≤ 200`.

- Пересечение быстрой SMA снизу вверх даёт сигнал покупки, сверху вниз — продажи.
- Сигналы рассчитываются последовательно, без использования будущих свечей.
  Начальное расположение средних не считается пересечением; равенство сохраняет предыдущую сторону.
- На графике отображаются обе SMA и точки пересечений; таблица показывает историю сигналов.
  Статус относится к последней доступной свече, её дата и время показаны отдельно.
- Для готовности требуется минимум `медленный период + 1` свеча. При нехватке истории
  показывается число доступных свечей. Периоды считаются по свечам, включая перерывы между сессиями.
- Расчёт обновляется вместе с историей примерно раз в минуту и кнопкой «Обновить».
  Параметры действуют в текущей вкладке и сбрасываются после перезагрузки страницы.

Это демонстрация сигналов, без отправки заявок, учёта позиции, комиссий и расчёта доходности.
Она использует историю, поступившую через существующий `vn.py EventEngine`;
торговый модуль `vnpy_ctastrategy` пока не подключён. В режиме `demo` данные смоделированы,
в `sandbox` и `readonly` используются свечи Т-Инвест за последние 24 часа.
...
`BaseGateway` для `MainEngine`, автоматического исполнения стратегий, бэктестов или отправки сделок.
Демонстрационная стратегия SMA рассчитывает только сигналы по истории.
